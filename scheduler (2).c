...
void policy_RR(int slice)
{
    ...
    int ran_any = 0; // means did any job run in the current full pass

    while (1) {
        ran_any = 0;

        // one full pass over the list, in order
        for (struct job *j = head; j != NULL; j = j->next) {
            // runnable now?
            if (j->end_time == -1 && j->arrival <= current_time && j->remaining > 0) {
                int start = current_time;
                // first time this job runs => responser time anchor
                if (j->start_time == -1)
                    j->start_time = current_time;

                int run = min(slice, j->remaining);
                // advance simulated time and job state; reduce remaining time
                j->remaining -= run;
                current_time += run;

                // if finished, stamp completion
                if (j->remaining == 0)
                    j->end_time = current_time;

                // emit the segment we just ran
                print_seg(start, j, current_time - start);

                ran_any = 1;
            }
        }

        // if every job is finished,  done, exit
        int done = 1;
        for (struct job *k = head; k != NULL; k = k->next) {
            if (k->end_time == -1) { done = 0; break; }
        }
        if (done) break;

        // nothing ran this cycle -> CPU idle; advance time by 1
        if (!ran_any) current_time++;
    }
...
void policy_LT(int slice)
{
    ...
    int firstA = next_arrival_after(-1);
    if (firstA == INT_MAX) { 
        printf("End of execution with LT.\n"); 
        return; 
    }
    t = firstA;
    // each iteration reps one lottery draw
    while (!all_done()) {
        int total_tickets = 0;
        int ready = 0;

        // sum tickets for arrived & unfinished jobs
        for (struct job *j = head; j; j = j->next)
            if (j->end_time < 0 && j->arrival <= t && j->remaining > 0)
                { total_tickets += j->tickets; ready++; }
        // if no job ready, jump to next arrivial if any
        if (ready == 0) {
            int na = next_arrival_after(t);
            if (na == INT_MAX) break;
            t = na; continue;
        }
        // draw winning ticket randomly
        int winning_ticket = rand() % total_tickets;
        int acc = 0;
        struct job *pick = NULL;

        for (struct job *j = head; j; j = j->next) {
            if (j->end_time < 0 && j->arrival <= t && j->remaining > 0) {
                acc += j->tickets;
                if (winning_ticket < acc) { pick = j; break; }
            }
        }
        assert(pick);
        // first run stamps start_time
        if (pick->start_time < 0) pick->start_time = t;
        // run for up to slice or remaining time
        int ran = min(slice, pick->remaining);
        print_seg(t, pick, ran);
        t += ran;
        pick->remaining -= ran;
        // if finished, stamp end time
        if (pick->remaining == 0) pick->end_time = t;
    }
